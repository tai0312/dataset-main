...
public class GamePanel extends JPanel implements Runnable, KeyListener, MouseListener, MouseMotionListener {
    ...
    private void updateGame() {
        ...
        if (panelPasses >= PANEL_PASS_COUNT) {
            ...
            panels.clear();
            obstacles.clear();
            boss = new Boss(WIDTH / 2 - 100, -100, calculateBossScore(), Color.RED);
        }
    }
    ...
    private void generatePanels() {
        ...
        for (int i = 0; i < PANEL_PASS_COUNT; i++) {
            String leftOperation = operations[random.nextInt(operations.length)];
            String rightOperation = operations[random.nextInt(operations.length)];
    ...
    private int calculateBossScore() {
        int maxScore = simulateMaxScore(score);
        return (int) (maxScore * 0.8); // 最大スコアの80%に設定
    }

    private int simulateMaxScore(int initialScore) {
        int simulatedScore = initialScore;
        String[] operations = {"+5", "+10", "*2", "*3", "-3", "-5"};

        for (int i = 0; i < PANEL_PASS_COUNT; i++) {
            String leftOperation = operations[random.nextInt(operations.length)];
            String rightOperation = operations[random.nextInt(operations.length)];
            int leftScore = evaluateOperation(leftOperation, simulatedScore);
            int rightScore = evaluateOperation(rightOperation, simulatedScore);
            simulatedScore = Math.max(leftScore, rightScore);
        }

        return simulatedScore;
    }

    private int evaluateOperation(String operation, int currentScore) {
        int result = currentScore;
        if (operation.startsWith("+")) {
            result += Integer.parseInt(operation.substring(1));
        } else if (operation.startsWith("*")) {
            result *= Integer.parseInt(operation.substring(1));
        } else if (operation.startsWith("-")) {
            result -= Integer.parseInt(operation.substring(1));
